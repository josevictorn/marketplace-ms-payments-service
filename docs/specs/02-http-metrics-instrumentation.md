# SPEC: Instrumentação de Métricas HTTP com Prometheus (Payments Service)

## Contexto
O serviço `payments-service` precisa expor um endpoint `GET /metrics` contendo métricas no formato do Prometheus. Atualmente, o Prometheus já está configurado na `observability-stack` para consultar esta rota a cada 15 segundos, mas o target está "down".
...
- **Autenticação:** O Prometheus realiza as consultas sem cabeçalhos de autorização. Logo, o endpoint deve estar público. Utilizar a anotação padrão do projeto para rotas públicas (ex: `@Public()` associado ao guard JWT local do serviço, ou `@SetMetadata` se não houver um decorator explícito criado).
...
7. O Prometheus da stack local deve reportar o `payments-service` como alvo com status **UP** na interface web.
